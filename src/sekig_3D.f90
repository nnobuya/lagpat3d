...
!    end do
    ...
    write(6,'(" Read Sekiguchi 3D data, job number =",i3," FMR level for tracer =",i2)') njob,lv_trc
...
!    allocate(ls34(lr34_min:lr34_max),le34(lr34_min:lr34_max),l34s(lr34_min:lr34_max))
     ...
     if( j1ma /= nx1 .or. k1ma /= nx2 .or. l1ma /= nx3 )then
      write(6,'(" Invalid grid number in sekig 3D =",6i3)')j1ma,nx1,k1ma,nx2,l1ma,nx3
      stop
     endif
...
!   endif
   do nn = 1, job34
         ...
         do l = ls34(lr),le34(lr),l34s(lr)
          do k = ks34(kr),ke34(kr),k34s(kr)
           do j = js34(jr),je34(jr),j34s(jr)
            ...
            read(134+i)qrho(nn,j1,k1,l1,lv),YE (nn,j1,k1,l1,lv),TEM(nn,j1,k1,l1,lv)  &
                      , VLX(nn,j1,k1,l1,lv),VLY(nn,j1,k1,l1,lv),VLZ(nn,j1,k1,l1,lv)  &
                      , hoge               ,hoge               ,hoge                 &
                      , hoge               ,hoge               ,hoge                 &
    ...
    !$omp end parallel
     ...
     write(6,'(" Read 3D sekig data, job =",i3," sub step =",i3," t [ms] =",es10.3)')njob,nn,time(nn)*t_uni*1.d3
...
!       deallocate(js34,je34,j34s,ks34,ke34,k34s,ls34,le34,l34s)
   ...
   if( iflag0 == 0 )then
    ...
    write(6,'(" Read Sekiguchi 3D data, FMR level for tracer =",i2)')lv_trc
